 ...
 *
 * Pass `null` to turn presence OFF: the hook clears the activity in Discord
 * (`clearActivity`) instead of pushing one. This makes a presence on/off toggle
 * a pure data concern — the caller passes `enabled ? activity : null` and the
 * effect does the right imperative thing — with no branching in the component.
 *
