...
export default defineConfig({
  ...
  pack: {
    ...
    platform: `node`,
    outDir: `electron`,
    // CRITICAL: outDir is the SOURCE dir (so the bundle sits beside main.mjs), so `clean` MUST be off — tsdown's default clean would delete electron/ (including the source preload.ts/main.mjs) before building.
    clean: false,
    dts: false,
    // Inline our app deps (so the sandbox has no node_modules imports), but keep `electron` EXTERNAL — it's provided by the Electron runtime, not npm. Bundling the `electron` npm package would inline its binary-locator shim (require child_process/fs) instead of the real runtime module, leaving contextBridge undefined.
  ...
  run: {
    tasks: {
      // Generate Varlock env types (env.d.ts, gitignored) before dev/build so TypeScript resolves `ENV.*`.
      ...
      // One-command launch: build the renderer + preload, then start Electron. The launcher scrubs ELECTRON_RUN_AS_NODE so it works from any terminal (incl. VS Code's, which sets it and would otherwise boot Electron as Node).
      start: {
