 ...
 * `hasUsableSession` predicate. These are imported by page *components* (which
 * can run on the client too), so they must NOT pull in the server-only pieces
 * of `session.ts` — `jose`, `waku/router/server`, or the `SESSION_SECRET` env
 * value. Keeping them in their own module severs that import edge so a client
 * bundle can never drag the secret in. `session.ts` re-exports these so server
 * code keeps a single import site. (Same split we landed on for the other
 * examples.)
 */
