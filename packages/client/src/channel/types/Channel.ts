...
export const channelSchema = object({
  ...
  parentId: nullish(string([minLength(1), maxLength(50)])),
  /** when the last pinned message was pinned. This may be null in events such as `GUILD_CREATE` when a message is not pinned. */
  lastPinTimestamp: nullish(string([isoTimestamp()])),
  ...
  messageCount: nullish(number([integer(), maxValue(50)])),
  /** an approximate count of users in a thread, stops counting at 50 */
  memberCount: nullish(number([integer(), maxValue(50)])),
  ...
  totalMessageSent: nullish(number([integer(), minValue(0)])),
  ...
  defaultThreadRateLimitPerUser: nullish(number([integer(), minValue(0)])),
...
export type Channel = Output<typeof channelSchema>;
