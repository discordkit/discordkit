import * as v from "valibot";
import { remove, type Fetcher, snowflake } from "@discordkit/core";

export const deleteGuildRoleSchema = v.object({
  guild: snowflake,
  role: snowflake
});

/**
 * ### [Delete Guild Role](https://discord.com/developers/docs/resources/guild#delete-guild-role)
 *
 * **DELETE** `/guilds/:guild/roles/:role`
 *
 * Delete a guild role. Requires the `MANAGE_ROLES` permission. Returns a `204 empty` response on success. Fires a Guild Role Delete Gateway event.
 *
 * > [!NOTE]
 * >
 * > This endpoint supports the `X-Audit-Log-Reason` header.
 */
export const deleteGuildRole: Fetcher<
  typeof deleteGuildRoleSchema,
  void,
  { auditLogReason: true }
> = async ({ guild, role }, options) =>
  remove(`/guilds/${guild}/roles/${role}`, options);
