...
export const memberSchema = v.object({
  ...
  nick: v.nullish<v.GenericSchema<string>>(v.pipe(v.string(), v.nonEmpty())),
  /** the member's guild avatar hash */
  avatar: v.nullish<v.GenericSchema<string>>(v.pipe(v.string(), v.nonEmpty())),
  /** the member's guild banner hash */
  banner: v.nullish<v.GenericSchema<string>>(v.pipe(v.string(), v.nonEmpty())),
  /** array of role object ids */
  roles: v.array<v.GenericSchema<string>>(snowflake),
  /** when the user joined the guild */
  joinedAt: v.pipe(v.string(), v.isoTimestamp()) as v.GenericSchema<string>,
  /** when the user started boosting the guild */
  premiumSince: v.nullish<v.GenericSchema<string>>(
  ...
  flags: asInteger(guildMemberFlag) as v.GenericSchema<number>,
  ...
  communicationDisabledUntil: v.nullish<v.GenericSchema<string>>(
    v.pipe(v.string(), v.isoTimestamp())
  ),
