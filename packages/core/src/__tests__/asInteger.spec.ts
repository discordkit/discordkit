...
import { integer, parse, pipe, object, getTitle, safeParse } from "valibot";
...
const mockUtils = new MockUtils(discord, {
  customMocks: (schema): unknown => {
    ...
    if (getTitle(schema) === getTitle(numberFlag)) {
      return MockUtils.flags(NumberFlags);
    }
    if (getTitle(schema) === getTitle(bigintFlag)) {
      return MockUtils.flags(BigIntFlags);
    }
    if (getTitle(schema) === getTitle(messageFlag)) {
      return MockUtils.flags(MessageFlag);
    }
  }
});
