import { any, getTitle, object, parse, safeParse } from "valibot";
...
export const mockUtils = new MockUtils(discord, {
  customMocks: (schema): unknown => {
    switch (getTitle(schema)) {
      case getTitle(permissionFlag):
        return MockUtils.flags(Permissions);
      case getTitle(snowflake):
        return MockUtils.uid.getUniqueID().toString();
      case undefined:
      default:
        throw new Error(`Unhandled custom schema detected!`);
    }
  }
});
