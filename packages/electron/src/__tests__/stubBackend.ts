 ...
 * main-process `registerDiscord` to boot the native client and wire its IPC
 * handlers + event subscriptions WITHOUT the real SDK. The electron bridge's job
 * is wiring; it doesn't need real native behavior to verify that every channel
 * is handled and that events broadcast.
 *
 ...
 * test scaffolding, so this local stub keeps the boundary at electron's edge.
 *
 * Typed structurally (not importing native's `FfiBackend`) so the dependency
 * stays one-directional; `registerDiscord` accepts it via `ClientConfig.backend`.
 */
