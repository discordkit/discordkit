/** The lobbies domain's bridge contract. Mirrors `@discordkit/native/lobbies`. */
...
/**
 * A serializable snapshot of a lobby — the bridge stand-in for the live `Lobby`
 * wrapper (native handles + methods can't cross the process boundary). Reads
 * return this; mutate the lobby with the id-keyed `lobbies.*` ops.
 */
...
/** The `lobbies` namespace on the webview bridge. */
