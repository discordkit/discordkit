...
/**
 * The reactive state of an in-flight async read. Every bridge read is a kkrpc
 * round-trip to the sidecar; this captures its lifecycle so a UI can render
 * loading/error/data without hand-rolling the bookkeeping.
 */
 ...
 *
 * This is the answer for the bridge's PULL-ONLY surfaces — reads with no event
 * stream to drive a true reactive signal (`relationships.list`, `voice.getCalls`,
 * `messages.getUserMessages`, `isSelfMuted`, …). Rather than dress a static read
 * up as auto-reactive, this models the thing that's actually hard across the
 * webview↔sidecar boundary: the async request lifecycle. It runs once on
 * creation, and a stale-run guard ensures only the latest `reload`'s result is
 * applied (an earlier slow reply can't clobber a newer one).
 *
...
export const asyncSignal = <T>(read: () => Promise<T>): AsyncSignal<T> => {
  ...
  // out-of-order reply from a superseded run can't overwrite fresher data.
  let latestRun = 0;
