...
export default defineConfig({
  ...
  lint: {
    ...
    overrides: [
      ...
      {
        files: [`*.stories.{j,t}s?(x)`, `*.config.?(m|c){j,t}s`],
        rules: {
  ...
  fmt: {
    ...
    ignorePatterns: [
      // CHANGELOG.md files are generated by Bumpy from .bumpy/*.md bodies.
      // Bumpy's renderer indents multi-paragraph entries as continuation
      // lines of a single list item, which oxfmt then flags as misformatted.
      // The format is dictated by the tool, not by us, and the file ships
      // with each package's release.
      `**/CHANGELOG.md`
    ]
  }
});
